...
module.exports = async (req, res) => {
  ...
  /** Append the binder's current key to a bare link. A binder with no key adds nothing. */
  const withVersion = (base, k) => (k ? `${base}?v=${encodeURIComponent(k)}` : base);
  // Accepts the hash form AND the integers shared before 20260826170000: those links are still out
  // there and must keep echoing the v they arrived with. Bounded and alphanumeric, so nothing
  // crafted can be reflected into og:url.
  const url = /^[a-z0-9]{1,16}$/i.test(v) ? `${SITE}/binder/${id}?v=${v}` : `${SITE}/binder/${id}`;
  ...
  const canonical = v ? url : withVersion(`${SITE}/binder/${id}`, binder.share_key);
