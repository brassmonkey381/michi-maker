# READ-ONLY: dumps the binder's pages to state\thirty-live.json for Claude to read.
#   powershell -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Brian\source\repos\tcgscan\michi-maker\state\dump-thirty-in-spreads.ps1"
...
Push-Location $repo
try { node "$PSScriptRoot\dump-thirty-in-spreads.mjs" $BinderId; $code = $LASTEXITCODE }
