# Deletes the connected-artwork binders from @fakemichi. Lists them first; -Apply actually removes.
#   powershell -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Brian\source\repos\tcgscan\michi-maker\state\connected-art-delete.ps1"
#   ...same, with -Apply          removes them
# Scoped to the three titles this import owns, on one account. Pages and slots go by cascade.
...
Push-Location $repo
try { node "$repo\scripts\connected-art\delete-binders.mjs" @argsList; $code = $LASTEXITCODE }
