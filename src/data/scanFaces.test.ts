...
test('an unclaimed pocket picks up an unworn photo rather than duplicating a worn one', () => {
  // Remove-and-readd: the re-added pocket lost its stamp, but the copy it could show is real
  // and no other pocket is wearing it. Three pockets, three scans owned: three faces is honest.
  const pool = world(copy('e1'), copy('e2'), copy('e3'));
  const slots = [slot({ sourceEntryId: 'e1' }), slot({ sourceEntryId: 'e2' }), slot()];
  const faces = allocateScanFaces(slots, pool);
...
test('a lot of three with one photo backs exactly three pockets', () => {
  ...
  const slots = [
    ...
    slot({ sourceEntryId: 'lot' }),
    slot(),
    slot(), // fourth pocket: the lot's capacity is spent
  ];
  const faces = allocateScanFaces(slots, pool);
...
test('a claim outranks a scavenge regardless of pocket order', () => {
  ...
  const pool = world(copy('e1'));
  const slots = [slot(), slot({ sourceEntryId: 'e1' })];
  const faces = allocateScanFaces(slots, pool);
...
test('legacy pre-stamp binder: unstamped pockets wear the owned scans, capped at what is owned', () => {
  // Wizard fills, composer pages, and guest-upgrade migration all produce stamp-less pockets for
  // owned cards. They keep their scans - the pool just runs dry at the honest count.
  const pool = world(copy('e1'), copy('e2'));
...
test('allocation is per card: one card cannot spend another card`s photos', () => {
  ...
  const slots = [slot(), slot({ cardId: 'charizard' })];
  const faces = allocateScanFaces(slots, pool);
  assert.equal(faces.get(slots[0].id), 'https://scans/e1.jpg');
...
test('non-card and cardless slots take nothing from the pool', () => {
  const pool = world(copy('e1'));
  const art = slot({ type: 'artwork' });
  const bare = slot({ cardId: undefined });
  const card = slot();
