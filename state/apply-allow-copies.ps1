# Applies supabase/migrations/20260916120000_allow_copies.sql to the live app project.
#   powershell -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Brian\source\repos\tcgscan\michi-maker\state\apply-allow-copies.ps1"
# Loads SUPABASE_ACCESS_TOKEN from analytics-studio\.env into the environment and never prints it.
# Safe to re-run: the column is added only if it is not there.
...
Push-Location $repo
try { node "$repo\scripts\apply-allow-copies.mjs"; $code = $LASTEXITCODE }
